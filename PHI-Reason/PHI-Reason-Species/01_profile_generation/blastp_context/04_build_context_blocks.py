...
Output format:
  ...
  ## Phylogenetic Cluster Context (proteome similarity via BLASTP, <ref_label>)
...
def build_block(neighbors: list[dict], host_gram: dict[str, str],
                ref_label: str = "train-set") -> dict | None:
    if not neighbors:
        return None
    lines = [
...
def build_block_no_host_label(neighbors: list[dict],
                              ref_label: str = "train-set") -> dict | None:
...
def main() -> int:
    ...
    ap.add_argument("--ref_label", default="train-set",
                    help="Reference DB label used in the context block header "
    ...
    for acc, hits in neighbors.items():
        if args.blast_context_mode == "no_host_label":
            block = build_block_no_host_label(hits, ref_label=args.ref_label)
        else:
            block = build_block(hits, host_gram, ref_label=args.ref_label)
        if block is None:
            n_none += 1
            continue
        out[acc] = block
