#!/usr/bin/env python3
"""
03_extract_neighbors.py — Aggregate diamond blastp TSV → top-K neighbor JSON.

Input:
  --tsv       DIAMOND blastp output (outfmt 6: qseqid sseqid pident length evalue bitscore).
              IDs are formatted as ACC__geneN (both query and subject).
  --pair_csv  Train-set phage-host pair CSV (acc,host — DB-side only)
  --out_json  Output JSON path

Aggregation:
  For each (query_phage, subject_phage) pair:
...
Self-hits (query acc == subject acc) are excluded. If a test-phage acc
coincides with a DB acc, the script warns.
