#!/usr/bin/env python3
"""
run_inference.py — parameterized driver for Cherry / HiC / VHDB evaluations.
=========================================================================
...
DEFAULT_URLS  = os.environ.get("PHI_OLLAMA_URLS", "http://127.0.0.1:11434")
...
async def call_ollama(session, url, system, user, model, num_predict, num_ctx, think=None, temperature=0.1):
    prompt = user
    if think is True:
        prompt = "/think\n" + user
    elif think is False:
...
async def infer_one(session, url_queue, phage_id, prof_dir, user_prefix,
                    ...
                    cache_dir, input_dir, counter, total, t_start, think=None, temperature=0.1):
    ...
    try:
        data = await call_ollama(session, url, SYSTEM_PROMPT, user_msg,
                                 model, num_predict, num_ctx, think=think, temperature=temperature)
    ...
    if done % SELFCHECK_EVERY == 0 or done == total:
        ...
        print(f"  [check {done:4d}/{total}] rate={rate:.2f}/s ETA={eta/60:.1f}min "
...
async def infer_batch(phage_ids, prof_dir, cache_dir, input_dir, host_names,
                      ...
                      concurrency, t_start, think=None, temperature=0.1):
    ...
    async with aiohttp.ClientSession() as s:
        tasks = [infer_one(s, q, pid, prof_dir, user_prefix, host_names,
                           ...
                           counter, len(phage_ids), t_start, think=think, temperature=temperature)
...
def write_summary(results_dir, exp_id, m, model, n_phages,
    ...
    md = f"""# Experiment Summary — {exp_id}

**Time**: {ts}
**Model**: {model}
**Phage count**: {n_phages}
**Elapsed**: {elapsed_h:.2f}h
**Errors/Cutoffs**: errors={n_err}  cutoffs={n_cutoff}
**Samples**: species-truth={m['n_species_truth']}  genus-truth={m['n_genus_truth']}
...
def main():
    ...
    if missing:
        print(f"  ⚠️  {len(missing)} phage(s) missing profile: {missing[:5]}...")
    ...
    print(f"  Phages    : {len(phage_ids)}  Hosts: {len(host_names)}")
    print(f"  Concurrency: {args.concurrency}  URLs: {urls}")
    print(f"  num_ctx={args.num_ctx}  num_predict={args.num_predict}  think={args.think}  temperature={args.temperature}")
    ...
    print("\n" + "=" * 72)
    print(f"  Done: {len(results)}  Elapsed: {elapsed_h:.2f}h")
    print(f"  errors={n_err}  cutoffs={n_cutoff}")
    print(f"  Samples: species={m['n_species_truth']}  genus={m['n_genus_truth']}")
